...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (existing) {
      // Mettre à jour les IDs Stripe et activer si besoin
    ...
    // Token reset pour créer le mot de passe (7 jours)
...
<div style="font-family:-apple-system,sans-serif;max-width:520px;margin:0 auto;padding:40px 32px;background:#f7f5f0;border-radius:20px;">
  ...
  <p style="color:#4a5568;font-size:15px;line-height:1.7;margin:0 0 24px;">
    Ton acces DadUp est pret. Il ne te reste qu'une etape : creer ton mot de passe.
  </p>
  ...
  <p style="color:#9aa0a8;font-size:13px;margin:0 0 8px;">Ce lien est valable 7 jours.</p>
