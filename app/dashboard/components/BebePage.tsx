    ...
    <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap' as const,gap:'12px'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          <button onClick={()=>setAvance((a:number)=>Math.max(-1,a-1))} disabled={avance<=-1} style={{fontSize:'12px',padding:'8px 16px',borderRadius:'20px',cursor:'pointer',background:'rgba(200,232,255,0.3)',color:'#0A2847',border:'1px solid rgba(46,95,138,0.15)',fontWeight:700,opacity:avance<=-1?0.4:1}}>← Précédente</button>
          <button onClick={()=>setAvance((a:number)=>Math.min(1,a+1))} disabled={avance>=1} style={{fontSize:'12px',padding:'8px 16px',borderRadius:'20px',cursor:'pointer',background:'rgba(200,232,255,0.3)',color:'#0A2847',border:'1px solid rgba(46,95,138,0.15)',fontWeight:700,opacity:avance>=1?0.4:1}}>Suivante →</button>
        </div>
      </div>
      ...
      {/* DÉVELOPPEMENT */}
      <div style={{background:'#fff',borderRadius:'20px',padding:'22px 24px',boxShadow:'0 2px 12px rgba(0,0,0,0.05)'}}>
      ...
      {data.organes?.length>0&&(
        <div style={{background:'rgba(200,232,255,0.2)',borderRadius:'20px',padding:'22px 24px',border:'1px solid rgba(46,95,138,0.15)'}}>
          <p style={{...LABEL_STYLE, marginBottom:'12px'}}>Organes cette semaine</p>
          <div style={{display:'flex',flexWrap:'wrap' as const,gap:'8px'}}>
            {data.organes.map((o:string,i:number)=>(
              <span key={i} style={{background:'rgba(200,232,255,0.3)',border:'1px solid rgba(46,95,138,0.15)',borderRadius:'20px',padding:'6px 14px',fontSize:'14px',fontWeight:700,color:'#0A2847'}}>{o}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{display:'flex',flexDirection:'column' as const,gap:'10px'}}>
            {data.faq.map((f:any,i:number)=>(
              <div key={i} style={{background:'rgba(200,232,255,0.3)',borderRadius:'14px',padding:'16px 18px',border:'1px solid rgba(46,95,138,0.15)'}}>
      ...
      {/* BÉBÉ EST NÉ */}
      {saReelle >= 35 && (
        <div style={{background:'rgba(221,208,255,0.3)',borderRadius:'16px',padding:'20px 24px',border:'1px solid rgba(107,79,187,0.15)',textAlign:'center' as const}}>
