...
export default function PostAccueil({C, moisBebe, dataBebe, joursRestants}: any) {
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', gap:'24px'}}>
      ...
      {/* HERO */}
      <div style={{background:'linear-gradient(135deg,#0a1f32,#1A3D5C)', borderRadius:'20px', padding:'24px 26px'}}>
        <p style={{color:'rgba(200,220,240,0.5)', fontSize:'11px', fontWeight:700, letterSpacing:'3px', textTransform:'uppercase' as const, margin:'0 0 6px'}}>
          Mois {moisBebe + 1}
        </p>
        <p style={{color:'#fff', fontSize:'22px', fontWeight:800, margin:'0 0 4px'}}>{dataBebe.titre}</p>
        <p style={{color:'rgba(255,255,255,0.45)', fontSize:'14px', margin:'0 0 14px'}}>{jours} jours de vie</p>
      </div>

      {/* POUR TOI */}
      {papa.length > 0 && (
        <div>
          <p style={{color:'#C04A1A', fontSize:'11px', fontWeight:700, letterSpacing:'2.5px', textTransform:'uppercase' as const, margin:'0 0 12px'}}>Pour toi ce mois-ci</p>
          <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
            {papa.map((item: any, i: number) => (
              <div key={i} style={{borderLeft:'3px solid #FFA07A', paddingLeft:'14px'}}>
                ...
                <p style={{color:'#5a6170', fontSize:'14px', lineHeight:1.65, margin:0}}>{item.contenu}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {activites.length > 0 && (
        <div>
          <p style={{color:'#2E5F8A', fontSize:'11px', fontWeight:700, letterSpacing:'2.5px', textTransform:'uppercase' as const, margin:'0 0 12px'}}>Activités avec bébé</p>
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
            {activites.map((item: any, i: number) => (
              <div key={i} style={{background:'#C8E8FF', borderRadius:'14px', padding:'14px 16px'}}>
                <p style={{color:'#1A3D5C', fontSize:'14px', fontWeight:700, margin:'0 0 5px'}}>{item.titre}</p>
                <p style={{color:'#2a5080', fontSize:'14px', lineHeight:1.6, margin:0}}>{item.contenu}</p>
              </div>
            ))}
          </div>
        </div>
