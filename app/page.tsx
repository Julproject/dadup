...
export default function TarifsPage() {
  const goToStripe = async () => {
    ...
    } catch(err) { alert('Erreur reseau. Reessaie.'); }
  };
    ...
    <main style={{minHeight:'100vh', background:'#ffffff', fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <section style={{background:'#1c2333', padding:'80px 40px'}}>
        <div style={{maxWidth:'800px', margin:'0 auto', textAlign:'center'}}>
          ...
          <p style={{color:'#9a8888', fontSize:'18px', lineHeight:1.7, margin:0}}>Tout est inclus. Acces immediat apres paiement.</p>
        </div>
      </section>
      ...
      <section style={{padding:'80px 40px', maxWidth:'960px', margin:'0 auto'}}>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'24px', marginBottom:'60px'}}>
          ...
          <div style={{border:'1.5px solid #e8e0d8', borderRadius:'24px', padding:'40px', background:'#f7f4ef'}}>
            ...
            <p style={{color:'#9a8888', fontSize:'14px', margin:'0 0 32px'}}>par mois, sans engagement</p>
            <button onClick={goToStripe} style={{width:'100%', background:'#1c2333', color:'#ffffff', border:'none', padding:'16px', borderRadius:'32px', fontSize:'15px', fontWeight:700, cursor:'pointer'}}>
              Commencer au mois
            </button>
          </div>

          <div style={{background:'#1c2333', borderRadius:'24px', padding:'40px', position:'relative', overflow:'hidden'}}>
            <div style={{position:'absolute', top:'18px', right:'18px', background:'#c8a060', color:'#1c2333', fontSize:'10px', fontWeight:800, padding:'4px 10px', borderRadius:'20px', letterSpacing:'1px'}}>RECOMMANDE</div>
            <p style={{color:'#c8a060', fontSize:'11px', fontWeight:700, letterSpacing:'3px', textTransform:'uppercase', margin:'0 0 20px'}}>Annuel</p>
            <p style={{color:'#f7f4ef', fontSize:'52px', fontWeight:900, margin:'0 0 4px', letterSpacing:'-1px', lineHeight:1}}>59,99 &#8364;</p>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:'10px', marginBottom:'28px'}}>
              {[
                ...
                <div key={i} style={{display:'flex', gap:'10px', alignItems:'flex-start'}}>
                  ...
                  <p style={{color:'#f7f4ef', fontSize:'13px', margin:0, lineHeight:1.5}}>{f}</p>
                </div>
              ))}
            </div>
            <button onClick={goToStripe} style={{width:'100%', background:'#c8a060', color:'#1c2333', border:'none', padding:'18px', borderRadius:'32px', fontSize:'16px', fontWeight:800, cursor:'pointer'}}>
              Commencer - 59,99 &#8364;/an
            </button>
      ...
      <section style={{background:'#f7f4ef', padding:'80px 40px'}}>
        <div style={{maxWidth:'800px', margin:'0 auto'}}>
          <h2 style={{fontSize:'32px', fontWeight:900, color:'#1c2333', margin:'0 0 40px', textAlign:'center', letterSpacing:'-0.5px'}}>Questions frequentes</h2>
          <div style={{display:'flex', flexDirection:'column'}}>
            {faq.map((item, i) => (
              <div key={i} style={{borderBottom:'1px solid #e8e0d8', padding:'24px 0'}}>
                <p style={{color:'#1c2333', fontSize:'15px', fontWeight:700, margin:'0 0 8px'}}>{item.q}</p>
                <p style={{color:'#9a8888', fontSize:'14px', lineHeight:1.65, margin:0}}>{item.r}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{background:'#1c2333', padding:'80px 40px', textAlign:'center'}}>
        <h2 style={{color:'#f7f4ef', fontSize:'38px', fontWeight:900, margin:'0 0 12px', letterSpacing:'-0.5px'}}>Pret a commencer ?</h2>
        <p style={{color:'#9a8888', fontSize:'16px', margin:'0 0 32px'}}>Rejoins les papas qui se preparent vraiment.</p>
        <button onClick={goToStripe} style={{background:'#c8a060', color:'#1c2333', border:'none', padding:'16px 40px', borderRadius:'32px', fontSize:'15px', fontWeight:800, cursor:'pointer'}}>
          Commencer - 59,99 &#8364;/an
        </button>
      </section>
