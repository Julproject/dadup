    ...
    <nav style={{background:C.white, borderBottom:`1px solid ${C.border}`, padding:'0 40px', display:'flex', alignItems:'center', justifyContent:'space-between', height:'68px', position:'sticky', top:0, zIndex:50}}>
      <div style={{display:'flex', alignItems:'center', gap:'48px'}}>
        ...
        <div style={{display:'flex', gap:'4px'}}>
          ...
          <a href="/temoignages" style={{color:C.dark, fontSize:'14px', fontWeight:700, padding:'8px 14px', borderRadius:'8px', textDecoration:'none', borderBottom:`2px solid ${C.gold}`}}>Temoignages</a>
        </div>
      </div>
...
export default function TemoignagesPage() {
  const temoignages = [
    {prenom:'Thomas', age:31, ville:'Lyon', semaine:'SA 28', texte:"J'avais l'impression d'etre un spectateur pendant toute la grossesse. DadUp m'a donne des outils concrets. A l'accouchement j'ai su exactement quoi faire. Ma compagne me dit encore que ca a tout change.", note:5},
    {prenom:'Maxime', age:28, ville:'Paris', semaine:'SA 34', texte:"Le module post-partum m'a sauve. Personne ne m'avait parle du baby blues. J'ai compris ce que vivait ma femme et j'ai pu etre la pour elle au lieu de paniquer.", note:5},
    {prenom:'Antoine', age:35, ville:'Bordeaux', semaine:'SA 20', texte:"29 euros pour tout ca c'est franchement honnete. J'aurais depense 10 fois plus en livres que j'aurais jamais finis. DadUp c'est pratique, personnalise, et je l'ai sur mon tel a tout moment.", note:5},
    {prenom:'Kevin', age:26, ville:'Marseille', semaine:'SA 12', texte:"Premier enfant, je ne savais vraiment pas par ou commencer. Le calendrier RDV m'a permis de comprendre chaque etape. Ma conjointe etait bluffee que je connaisse tout ca.", note:5},
    {prenom:'Julien', age:33, ville:'Nantes', semaine:'SA 36', texte:"La valise etait prete 3 semaines avant. On n'a rien oublie. Le jour du depart a la maternite j'etais stresse comme tout le monde mais au moins j'etais organise.", note:5},
    {prenom:'Nicolas', age:30, ville:'Toulouse', semaine:'SA 40', texte:"Bebe est arrive. Je voulais juste dire merci. Le guide accouchement m'a donne confiance. J'etais la, vraiment la, du debut a la fin. C'est le plus beau jour de ma vie.", note:5},
  ];
    ...
    <main style={{minHeight:'100vh', background:C.white, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <section style={{padding:'80px 40px', maxWidth:'1200px', margin:'0 auto'}}>
        <div style={{display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px'}}>
          {temoignages.map((t, i) => (
            <div key={i} style={{background: i === 1 || i === 4 ? C.dark : C.cream, borderRadius:'20px', padding:'28px', border: i === 1 || i === 4 ? 'none' : `1px solid ${C.border}`}}>
              <div style={{display:'flex', gap:'4px', marginBottom:'16px'}}>
                {[...Array(t.note)].map((_, j) => (
                  <span key={j} style={{color:C.gold, fontSize:'16px'}}>★</span>
                ))}
              </div>
              <p style={{color: i === 1 || i === 4 ? C.white : C.dark, fontSize:'15px', lineHeight:1.7, margin:'0 0 20px', fontStyle:'italic'}}>"{t.texte}"</p>
